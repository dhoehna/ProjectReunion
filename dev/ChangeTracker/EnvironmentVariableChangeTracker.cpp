...
namespace winrt::Microsoft::ProjectReunion::implementation
{
    ...
    HRESULT EnvironmentVariableChangeTracker::TrackChange(std::function<HRESULT(void)> callback)
    {
        if (m_ShouldTrackChange)
        {
            ...
            if ((m_Key.compare(L"Path") == 0) ||
                m_Key.compare(L"PATHEXT") == 0)
            {
                std::wstring pathOrPathExtChanges = GetPathOrPathExtChangesFromRegistry();

                std::wstring newPathChanges = pathOrPathExtChanges.append(m_Value.c_str()).append(L";");
